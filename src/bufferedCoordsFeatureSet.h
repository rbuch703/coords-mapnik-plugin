#ifndef BUFFERED_COORDS_FEATURESET_H
#define BUFFERED_COORDS_FEATURESET_H
...
class BufferedCoordsFeatureSet : public mapnik::Featureset
{
public:
    enum GEOMETRY_TYPE {POINT, LINE, POLYGON};

    // this constructor can have any arguments you need
    BufferedCoordsFeatureSet(mapnik::box2d<double> const& box, std::string const& encoding, std::string path, std::set<std::string> propertyNames);

    // desctructor
    virtual ~BufferedCoordsFeatureSet();
...
//    void markAsReturnedBefore(OSM_ENTITY_TYPE entityType, uint64_t entityId);
    void readFiles(std::string path, const mapnik::box2d<double> &queryBounds, 
                   mapnik::box2d<double> tileBounds);
                            
    void loadFileContents( const std::string &fileName);
private:
    std::vector<std::pair<int8_t, GenericGeometry*>> loadedGeometries;
    bool isRoad;
    Envelope box_;
